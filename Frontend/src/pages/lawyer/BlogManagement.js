...
const BlogManagement = () => {
  ...
  const fetchBlogs = async () => {
    try {
      setLoading(true);
      // Try lawyer-specific endpoint first, fallback to general endpoint with filtering
      try {
        ...
        // Get current user from token/localStorage
        const user = JSON.parse(localStorage.getItem('user') || '{}');
        const userBlogs = blogsData.filter(blog => blog.author_id === user.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('📝 Creating blog with data:', submitData);
